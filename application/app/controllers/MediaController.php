	...
	public function store()
	{
		if(!Auth::guest() && Auth::user()->active){
			...
			if ($validation->passes() && $valid_media)
			{
				...
				return Redirect::to('admin?section=media')->with(array('note' => 'Cập nhật thành công!', 'note_type' => 'success') );
			}

			return Redirect::to('admin?section=media')->with(array('note' => Lang::get('lang.error_uploading'), 'note_type' => 'error') );
		...
		if(Auth::user()->admin == 1 || Auth::user()->id == $media->user_id){
			...
			try{
				...
				if(isset($input['pic_url'])){
					$media->pic_url = ImageHandler::uploadImage(Input::file('pic_url'), 'images');
				}else
					$media->pic_url = $media->pic_url;
				...
				$media->slug = Helper::slugify($media->title);

				// $slugexist = Media::where('slug', '=', $sSlug)->first();
				// $increment = 1;
				// while(isset($slugexist->id)){
					// $media->slug = $input['slug'] . '-' . $increment;
					// $slugexist = Media::where('slug', '=', $input['slug'])->first();
					// $increment += 1;
				// }
				$media->category_id = $input['category_id'];
				$media->location = $input['location'];
				$media->link_url = htmlspecialchars($input['link_url']);
				// $media->tags = htmlspecialchars($input['tags']);
				
				$media->save();
				return Redirect::to($input['redirect'])->with(array('note' => 'Cập nhật thành công', 'note_type' => 'success') );
			} catch(Exception $e){
